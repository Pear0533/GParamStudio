...
public partial class GParamStudio : Form
{
    ...
    private static void EnsureMacAddressIntegrity()
    {
        if (NetworkInterface.GetAllNetworkInterfaces().Any(nic => addresses.Contains(nic.GetPhysicalAddress().ToString().ToLower()))) return;
        ShowInformationDialog("Your machine is not authorized to use this program.");
    ...
    private static void SaveGPARAMFile(string fileName)
    {
        ...
        if (fileName.EndsWith(".dcx")) File.WriteAllBytes(gparamFileName, DCX.Compress(File.ReadAllBytes(fixedGParamFileName), gparamArchiveType));
    }
    ...
    private void ParamsBoxNodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
    {
        if (e.Node.Parent != null)
        {
            ...
            int[] valueInfo = GetValueInfoFromParamValInfoList(e.Node);
            // NOTE: CONVERT VALUE DEPENDING ON TYPE USING SWITCH CASE
            var colorValues = (Vector4)gparam.Groups[valueInfo[0]].Params[valueInfo[1]].Values[valueInfo[2]];
            string valueStr = BitConverter.ToString(BitConverter.GetBytes(colorValues.X)).Replace("-", " ")
                + " "
                + BitConverter.ToString(BitConverter.GetBytes(colorValues.Y)).Replace("-", " ")
                + " "
                + BitConverter.ToString(BitConverter.GetBytes(colorValues.Z)).Replace("-", " ")
                + " "
                + BitConverter.ToString(BitConverter.GetBytes(colorValues.W)).Replace("-", " ")
                + " 00 00 00 00";
            var scanner = new Scanner(process, process.Handle, valueStr);
            scanner.setModule(process.MainModule);
            ulong valueAddress = scanner.FindPattern();
            Console.WriteLine(valueAddress);
            scanner.writeBytes(process, valueAddress, new byte[4]);
        }
        UpdateInteractiveControl(e.Node);
    }
